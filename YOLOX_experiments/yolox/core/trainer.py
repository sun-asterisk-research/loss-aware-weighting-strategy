...
class Trainer:
    ...
    def after_iter(self):
        ...
        if (self.iter + 1) % self.exp.print_interval == 0:
            ...
            reg_weight_meter = self.meter.get_filtered_meter("reg_weight")
            reg_weight_str = ", ".join(
                ["{}: {:.3f}s".format(k, v.avg) for k, v in reg_weight_meter.items()]
            )
            loss_str = ", ".join(
            ...
            if self.rank == 0:
                if self.args.logger == "wandb":
                    ...
                    self.wandb_logger.log_metrics({"reg_weight": self.meter["reg_weight"].latest})
