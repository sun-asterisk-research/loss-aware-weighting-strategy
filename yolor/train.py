...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        logger.info(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'total', 'targets', 'img_size', 'reg_w'))
        if rank in [-1, 0]:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if rank in [-1, 0]:
                ...
                s = ('%10s' * 2 + '%10.4g' * 7) % (
                    '%g/%g' % (epoch, epochs - 1), mem, *mloss, targets.shape[0], imgs.shape[-1], model.reg_weight.item())
                ...
                if plots and ni < 3:
                    ...
                    wandb.log({'reg_weight':model.reg_weight.item()})
